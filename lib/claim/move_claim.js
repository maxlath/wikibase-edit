...
module.exports = async (params, config, API) => {
  ...
  if (currentId === id) {
    ...
    currentEntityData.claims.push(claim)
    ...
    const removeClaimRes = await API.entity.edit(currentEntityData, config)
    const addClaimRes = await API.entity.edit(targetEntityData, config)
