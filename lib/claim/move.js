...
module.exports = async (params, config, API) => {
  ...
  if (propertyDatatype !== currentPropertyDatatype) {
    ...
    propertiesDatatypesDontMatch({ movedClaims, typeConverterKey })
  }
...
const propertiesDatatypesDontMatch = ({ movedClaims, typeConverterKey }) => {
  const convertType = claimTypeConversions[typeConverterKey]
  if (convertType) {
    for (const claim of movedClaims) {
      if (claimWithValue(claim)) {
        try {
          convertType(claim)
        } catch (err) {
          const errMessage = `properties datatype don't match and ${typeConverterKey} type conversion failed: ${err.message}`
          throw error_.new(errMessage, 400, { typeConverterKey })
        }
      }
    }
  } else {
