...
module.exports = config => loginCookies => {
  const { instanceApiEndpoint, credentials, userAgent } = config
  const { oauth: oauthTokens } = credentials

  const url = `${instanceApiEndpoint}?action=query&meta=tokens&format=json`
...
const parseTokens = (loginCookies, instanceApiEndpoint) => async res => {
