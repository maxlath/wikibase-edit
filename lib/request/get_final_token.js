...
module.exports = config => async loginCookies => {
  const { instanceApiEndpoint, credentials, userAgent, letAgentHandleLoginCookies = false, origin } = config
  const { oauth: oauthTokens } = credentials

  let url = `${instanceApiEndpoint}?action=query&meta=tokens&type=csrf&format=json`
  ...
  } else if (!letAgentHandleLoginCookies) {
