...
module.exports = config => {
  ...
  let loginUrl = `${config.instanceApiEndpoint}?action=login&format=json`
  if (config.origin) loginUrl += `&origin=${config.origin}`
...
const getSessionCookies = (loginUrl, config, headers) => async ({ cookies, token: lgtoken }) => {
  const { letAgentHandleLoginCookies = false } = config
  const { username: lgname, password: lgpassword } = config.credentials
  const body = qs.stringify({ lgname, lgpassword, lgtoken })

  if (!letAgentHandleLoginCookies) {
  ...
  if (letAgentHandleLoginCookies) return ''
