...
const actionPost = (action, data, config) => authData => {
  const { instance, userAgent, bot, summary, baserevid, tags, maxlag, anonymous, autoRetry, origin, letAgentHandleLoginCookies } = config
  ...
  const afterRequest = letAgentHandleLoginCookies ? logoutIfNeeded(instance, data.token) : identity
...
const logoutIfNeeded = (instance, token) => body => {
  return logout(instance, token)
  .then(() => body)
}
