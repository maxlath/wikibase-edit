...
const actionPost = async ({ action, data, config, authData }) => {
  const { instance, instanceApiEndpoint, userAgent, bot, summary, baserevid, tags, maxlag, anonymous } = config
  ...
  if (config.credentials?.browserSession && globalThis.location?.origin !== instance) {
    query.origin = '*'
  }
