...
export default async (data, properties, instance, config) => {
  ...
  if (rawMode) {
    ...
    if (statements) params.data[statementsKey] = statements
    ...
    if (statements) params.data[statementsKey] = format.claims(statements, properties, instance, reconciliation, existingClaims)
