...
module.exports = {
  ...
  claims: (claims, properties, instance) => {
    if (!properties) throw error_.new('expected properties')
    return Object.keys(claims).reduce(formatClaim(claims, properties, instance), {})
  },
  sitelinks: sitelinks => {
    const obj = {}
    Object.keys(sitelinks).forEach(site => {
      validate.site(site)
      let title = sitelinks[site]
      if (title === null) {
...
const formatClaim = (claims, properties, instance) => (obj, property) => {
  if (!properties) throw error_.new('expected properties')
