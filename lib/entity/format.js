...
module.exports = {
  ...
  claims: (claims, properties) => {
    if (!properties) throw new Error('expected properties')
    return Object.keys(claims).reduce(formatClaim(claims, properties), {})
  },
  sitelinks: sitelinks => {
    const obj = {}
...
const formatClaim = (claims, properties) => (obj, property) => {
  if (!properties) throw new Error('expected properties')
  validate.property(property)
  const values = _.forceArray(claims[property])
  obj[property] = obj[property] || []
  obj[property] = values.map(value => buildClaim(property, properties, value))
  return obj
}
