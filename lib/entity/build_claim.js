...
const fullClaimBuilder = params => {
  ...
  if (qualifiers) {
    claim.qualifiers = _.map(qualifiers, buildPropSnaks('qualifier', properties, instance))
  }
...
const buildReference = (properties, instance) => reference => {
  ...
  const snaksPerProperty = _.map(referenceSnaks, buildPropSnaks('reference', properties, instance))
  const snaks = _.flatten(_.values(snaksPerProperty))
  return { snaks, hash }
}

const buildPropSnaks = (label, properties, instance) => (prop, propSnakValues) => {
