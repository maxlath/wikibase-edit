...
module.exports = config => {
  ...
  API = {
    ...
    post: (action, data) => {
      const tryIt = () => {
        return authDataPromise
        .then(actionPost(log, instance.base, userAgent, action, data, oauth, bot))
        .then(Log(`${action} res`))
...
const actionPost = (log, base, userAgent, action, data, oauth, bot) => authData => {
  const { cookie, token } = authData
  data.token = token

  const url = _.buildUrl(base, { action, format: 'json', bot })
  const params = {
    url,
    oauth,
    headers: {
